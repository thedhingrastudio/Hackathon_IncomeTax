...
export default function UpcomingReminders({ waitingForReview = false }: { waitingForReview?: boolean }) {
  ...
  return <section aria-describedby="upcoming-reminders-disclosure" aria-labelledby="upcoming-reminders-title" className="assistance-upcoming">
    ...
    <ol>{reminders.map((reminder) => {
      const label = new Intl.DateTimeFormat("en-IN", { day: "2-digit", month: "short", timeZone: "UTC" }).format(new Date(`${reminder.date}T00:00:00Z`)).toUpperCase();
      return <li key={reminder.date}><time dateTime={reminder.date}>{label}</time><div><strong>{reminder.title}</strong>{reminder.context ? <span>{reminder.context}</span> : null}</div></li>;
    })}</ol>
