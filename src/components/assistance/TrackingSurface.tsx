...
import { Check, Circle, Clock3, ShieldCheck } from "lucide-react";
...
export default function TrackingSurface({ headingRef }: { headingRef: RefObject<HTMLHeadingElement | null> }) {
  ...
  return <section aria-labelledby="assistance-tracking-title" className="assistance-tracking">
    <header className="assistance-tracking-header"><span aria-hidden="true"><Clock3 /></span><div><p className="assistance-kicker">Outstanding demand · {formatIndianCurrency(taxCase.demandAmount, taxCase.currency)}</p><h2 id="assistance-tracking-title" ref={headingRef} tabIndex={-1}>Waiting for Income Tax review</h2><p>Nothing you need to do right now.</p></div></header>
    <div className="assistance-case-current" role="status"><ShieldCheck aria-hidden="true" /><div><strong>Your part is complete</strong><p>Both requests were submitted and linked to this case.</p></div><span>In review</span></div>
    ...
    <Link className={buttonVariants({ className: "app-action app-action-secondary assistance-view-case", size: "lg" })} href={`/case/${taxCase.caseId}`}>View full case</Link>
  </section>;
}
