...
export default function DemandResponseWorkflow({ demand, payment }: { demand: OutstandingDemand; payment: TaxPayment }) {
  ...
  if (step === "submitted" && result) return <section className="workflow-panel" aria-labelledby="submitted-title"><div className="portal-alert portal-alert--success response-success" role="status"><div><h1 id="submitted-title" ref={headingRef} tabIndex={-1}>Response submitted</h1><p>Your response to the {formatIndianCurrency(demand.amount, demand.currency)} outstanding demand has been recorded.</p></div></div><div className="portal-surface"><div className="portal-surface__body"><dl className="review-list"><div><dt>Response reference</dt><dd>{result.reference}</dd></div><div><dt>Assessment Year</dt><dd>{formatAssessmentYear(demand.assessmentYear)}</dd></div><div><dt>Response status</dt><dd>Submitted</dd></div></dl></div></div><div className="workflow-actions"><Link className="app-action app-action-primary" href="/pending-actions">Back to Pending Actions</Link><Link className="app-action app-action-secondary" href="/dashboard">Back to Dashboard</Link></div></section>;

  const activeStep = step === "response" ? 1 : step === "details" ? 2 : 3;
  return <section className="workflow-panel" aria-labelledby="workflow-title">
    <ol className="response-stepper" aria-label={`Step ${activeStep} of 3`}><li className={activeStep > 1 ? "is-complete" : "is-current"} aria-current={activeStep === 1 ? "step" : undefined}><span aria-hidden="true">{activeStep > 1 ? "✓" : "1"}</span>Response</li><li className={activeStep > 2 ? "is-complete" : activeStep === 2 ? "is-current" : "is-future"} aria-current={activeStep === 2 ? "step" : undefined}><span aria-hidden="true">{activeStep > 2 ? "✓" : "2"}</span>Details</li><li className={activeStep === 3 ? "is-current" : "is-future"} aria-current={activeStep === 3 ? "step" : undefined}><span aria-hidden="true">3</span>Review</li></ol>
    ...
    {step === "response" ? <><header className="workflow-heading"><p className="eyebrow">Step 1 of 3</p><h1 id="workflow-title" ref={headingRef} tabIndex={-1}>How do you want to respond?</h1><p>Select one response. Income Tax will record the option you choose.</p></header><fieldset className={`response-options ${errors.choice ? "has-error" : ""}`} aria-describedby={errors.choice ? "choice-error" : undefined}><legend className="visually-hidden">Demand response</legend>{responseOptions.map((option) => <label className="response-option" key={option.value}><input type="radio" name="response-choice" value={option.value} checked={draft.choice === option.value} onChange={() => update("choice", option.value)} /><span className="radio-indicator" aria-hidden="true" /><span className="radio-content"><strong>{option.title}</strong><small>{option.description}</small></span></label>)}</fieldset>{errors.choice ? <p className="field-error" id="choice-error">{errors.choice}</p> : null}<div className="workflow-actions"><button className="app-action app-action-primary" type="button" onClick={continueFromResponse}>Continue</button><Link className="app-action app-action-tertiary" href="/pending-actions/demand">Cancel</Link></div></> : null}
...
function DetailsStep({ draft, demand, errors, headingRef, update, onBack, onContinue }: { draft: ResponseDraft; demand: OutstandingDemand; errors: Errors; headingRef: RefObject<HTMLHeadingElement | null>; update: <K extends keyof ResponseDraft>(field: K, value: ResponseDraft[K]) => void; onBack: () => void; onContinue: () => void }) {
  if (draft.choice === "demand_correct") return <><header className="workflow-heading"><p className="eyebrow">Step 2 of 3</p><h1 id="workflow-title" ref={headingRef} tabIndex={-1}>Payment is the next step</h1><p>You have agreed that the outstanding amount is payable.</p></header><div className="portal-surface"><div className="portal-surface__body"><dl className="review-list"><div><dt>Outstanding amount</dt><dd>{formatIndianCurrency(demand.amount, demand.currency)}</dd></div><div><dt>Assessment Year</dt><dd>{formatAssessmentYear(demand.assessmentYear)}</dd></div></dl></div></div><div className="workflow-actions"><Link className="app-action app-action-primary" href="/pending-actions/demand/pay">Continue to payment</Link><button className="app-action app-action-secondary" type="button" onClick={onBack}>Back</button></div></>;
  return <><header className="workflow-heading"><p className="eyebrow">Step 2 of 3</p><h1 id="workflow-title" ref={headingRef} tabIndex={-1}>{draft.choice === "already_paid" ? "Provide payment details" : "Tell us why you disagree"}</h1><p>{draft.choice === "already_paid" ? "Enter the payment information you want recorded with this response." : "Select a reason and enter the amount you dispute."}</p></header>
    {draft.choice === "already_paid" ? <div className="form-grid"><InputField label="Assessment Year" value={formatAssessmentYear(demand.assessmentYear)} readOnly /><InputField label="Amount (₹)" value={draft.paymentAmount} type="number" error={errors.paymentAmount} onChange={(value) => update("paymentAmount", value)} /><InputField label="Payment date" value={formatIndianDate(draft.paymentDate)} readOnly error={errors.paymentDate} /><InputField label="Challan reference" value={draft.challanReference} error={errors.challanReference} onChange={(value) => update("challanReference", value)} /><input type="hidden" name="paymentDate" value={draft.paymentDate} /></div> : <><fieldset className={`reason-options ${errors.disagreementReason ? "has-error" : ""}`} aria-describedby={errors.disagreementReason ? "reason-error" : undefined}><legend>Reason for disagreement</legend>{disagreementReasons.map((reason) => <label className="radio-option" key={reason.value}><input type="radio" name="disagreement-reason" checked={draft.disagreementReason === reason.value} onChange={() => update("disagreementReason", reason.value)} /><span className="radio-indicator" aria-hidden="true" /><span className="radio-content">{reason.label}</span></label>)}</fieldset>{errors.disagreementReason ? <p className="field-error" id="reason-error">{errors.disagreementReason}</p> : null}<div className="form-grid">{draft.disagreementReason === "other" ? <InputField label="Short explanation" value={draft.otherReason} error={errors.otherReason} onChange={(value) => update("otherReason", value)} /> : null}<InputField label="Amount disputed (₹)" value={draft.disputedAmount} type="number" error={errors.disputedAmount} onChange={(value) => update("disputedAmount", value)} /></div></>}
    <div className="workflow-actions"><button className="app-action app-action-primary" type="button" onClick={onContinue}>Continue</button><button className="app-action app-action-secondary" type="button" onClick={onBack}>Back</button></div></>;
}
...
function ReviewStep({ draft, demand, headingRef, onBack, onSubmit }: { draft: ResponseDraft; demand: OutstandingDemand; headingRef: RefObject<HTMLHeadingElement | null>; onBack: () => void; onSubmit: () => void }) {
  ...
  return <><header className="workflow-heading"><p className="eyebrow">Step 3 of 3</p><h1 id="workflow-title" ref={headingRef} tabIndex={-1}>Review your response</h1><p>Check this information before submitting your response.</p></header><div className="portal-surface"><div className="portal-surface__body"><dl className="review-list">{details.map((detail) => <div key={detail.label}><dt>{detail.label}</dt><dd>{detail.value}</dd></div>)}</dl></div></div><div className="portal-alert portal-alert--info consequence-message" role="note"><strong>Nothing has been submitted yet.</strong></div><div className="workflow-actions"><button className="app-action app-action-primary" type="button" onClick={onSubmit}>Confirm and submit</button><button className="app-action app-action-secondary" type="button" onClick={onBack}>Back</button></div></>;
}
