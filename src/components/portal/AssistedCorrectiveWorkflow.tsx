...
function FactsCard({ title, children }: { title: string; children: React.ReactNode }) {
  return <section className="portal-surface corrective-card" aria-labelledby={`corrective-${title.toLowerCase().replaceAll(/[^a-z0-9]+/g, "-")}`}><div className="portal-surface__header"><h2 id={`corrective-${title.toLowerCase().replaceAll(/[^a-z0-9]+/g, "-")}`}>{title}</h2></div><div className="portal-surface__body">{children}</div></section>;
}
...
function RectificationReview({ draft, headingRef, onSubmit }: { draft: RectificationDraft; headingRef: React.RefObject<HTMLHeadingElement | null>; onSubmit: () => void }) {
  return <section className="workflow-panel corrective-workflow" aria-label="Tax credit correction review">
    ...
    <div className="portal-alert portal-alert--info consequence-message" role="note"><strong>Nothing has been submitted yet.</strong><p>You are requesting a Tax Credit Mismatch Correction for {formatIndianCurrency(draft.amount, draft.currency)} using challan {draft.challanReference} because the payment was missing from the processed return.</p></div>
    <div className="workflow-actions"><button className="app-action app-action-primary" type="button" onClick={onSubmit}>Confirm and submit correction</button><Link className="app-action app-action-secondary" href="/pending-actions/demand/assist">Back</Link></div>
  </section>;
}
...
function RectificationSubmitted({ draft, submission, headingRef, onContinue }: { draft: RectificationDraft; submission: RectificationSubmission; headingRef: React.RefObject<HTMLHeadingElement | null>; onContinue: () => void }) {
  return <section className="workflow-panel corrective-workflow" aria-label="Correction submitted">
    <div className="portal-alert portal-alert--success response-success" role="status"><div><h1 ref={headingRef} tabIndex={-1}>Correction submitted</h1><p>Your correction request has been recorded.</p></div></div>
    ...
    <section className="next-action" aria-labelledby="next-action-title"><h2 id="next-action-title">Next: respond to the demand</h2><p>Now that the correction request exists, you can respond to the {formatIndianCurrency(draft.amount, draft.currency)} outstanding demand.</p><button className="app-action app-action-primary" type="button" onClick={onContinue}>Review demand response</button></section>
  </section>;
}
...
function DemandResponseReview({ draft, headingRef, onBack, onSubmit }: { draft: AssistedDemandResponseDraft; headingRef: React.RefObject<HTMLHeadingElement | null>; onBack: () => void; onSubmit: () => void }) {
  return <section className="workflow-panel corrective-workflow" aria-label="Assisted demand response review">
    ...
    <div className="portal-alert portal-alert--info consequence-message" role="note"><strong>Nothing has been submitted yet.</strong><p>You are confirming this response and the full disputed amount shown above.</p></div>
    <div className="workflow-actions"><button className="app-action app-action-primary" type="button" onClick={onSubmit}>Confirm and submit response</button><button className="app-action app-action-secondary" type="button" onClick={onBack}>Back</button></div>
  </section>;
}
...
function ValidationFailure({ errors }: { errors: string[] }) {
  return <section className="workflow-panel"><div className="portal-alert portal-alert--warning" role="alert"><div><h1>We could not verify this request</h1><p>Some required payment or tax-record information could not be verified, so nothing has been submitted.</p>{errors.length ? <p className="supporting-text">Verification code: {errors.join(", ")}</p> : null}</div></div><div className="workflow-actions"><Link className="app-action app-action-primary" href="/pending-actions/demand/assist">Back to explanation</Link><Link className="app-action app-action-secondary" href="/pending-actions/demand">View demand records</Link></div></section>;
}
