  ...
  <div className="site-shell">
    ...
    <header><div className="government-masthead"><div className="portal-container government-masthead__inner"><span>भारत सरकार</span><span aria-hidden="true">|</span><span>Government of India</span></div></div>
      <nav className="portal-navbar" aria-label="Primary navigation"><div className="portal-navbar__inner portal-container">
        ...
        <button ref={menuButtonRef} className="portal-menu-button menu-button" type="button" aria-expanded={open} aria-controls="mobile-primary-menu" onClick={() => setOpen((current) => !current)}><span aria-hidden="true">☰</span> Menu</button>
        <ul className="portal-links desktop-portal-links">{navigation.map(([label, href]) => { const isCurrent = pathname === href || (href !== "/" && pathname.startsWith(`${href}/`)); return <li key={href}><Link aria-current={isCurrent ? "page" : undefined} href={href}>{label}</Link></li>; })}</ul>
        <div className="taxpayer-summary"><span aria-hidden="true">{taxpayerInitials}</span><div><strong>{taxpayerName}</strong><small>Individual taxpayer</small></div></div>
